...
<body>
<div id="center">
	<h1>W+K Communicator</h1>
	<form name="communicator" action="save.php" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
		<div class="label">姓名 / Name:</div>
		<input type="text" name="name" maxlength=100 />
		<div class="label">讯息 / Message:</div>
		<textarea name="content" maxlength=100></textarea>
		<input id="btn_pic" type="button" value="加图片 / Add Picture" onclick="showUploadUI()" />
		<div id="upload_ui">
			<div class="or">上传图片 / Upload Image:</div>
			<input type="file" name="file" id="file" />
			<div class="or">或图片链结 / Or Enter Image URL</div>
			<input type="text" name="url" id="url" />
		</div>
		<input type="submit" value="发送 / Submit" />
	</form>
	You can also post by Instagram with hashtag #wkshare
	<hr />
	...
	while($row = mysql_fetch_array($result)) {
		...
		echo '<div class="' . $class . '">' . htmlspecialchars($row['name']) . ': ' . htmlspecialchars($row['content']) .
			$imghtml . 
			' <div class="timestamp">[' . $date->format('Y-m-d g:i:s a') . ' via ' . $row['platform'] . ']</div>' .
			'</div>';
	}
